...
| `review-opus` | claude (`--model claude-opus-5-5 --effort high`) | Astra-equivalent reviewer: reviews fable-implemented work when no Astra lane is available, or stands in for review-fable on a Fable-limit stall |
...
Spawn lazily on first task; `herd spawn` is idempotent — a live same-kind lane is adopted (also the resume path). Each lane gets its own tab; `--worktree` gives it a managed worktree on branch `lane/<name>` instead. `HERD_WORKSPACE=<id>` targets new tab lanes at a specific herdr workspace.

## Lane lifecycle (the whole loop)

```text
herd spawn <lane> --kind <claude|codex|pi> [--worktree] [--cwd DIR] [--profile NAME] [--no-nudge] [-- extra-args]
...
**spawn** pre-seeds the CLI's own trust store for the lane's cwd (codex `~/.codex/config.toml`, claude `~/.claude.json`; pi via `--approve`) so folder-trust dialogs never appear; anything pretrust can't handle falls to the exit-3 path. It also gitignores `.herd/` in the project root.
...
A fresh session resumes from the ledger: re-adopt live lanes with `herd spawn <lane> --kind <kind>` (idempotent), confirm each lane's model banner, re-attach a background `herd watch <lane> --resume` for every lane in implementing/review/fixing (tokens persist in the ledger, and consumed tokens return `already-reported`), and pick up the review obligations the states imply. Never re-send a slice a live lane already has.
